...
| Editorial | Library has Crosswords and Wordle tabs. E1 three-step import (7 accepted, 2 rejected, batch in Operations). E2 new game and duplicate as draft. E3 save increments version. E4 failing validation on CW-2264 and D5-0923, issue click focuses the cell or clue, fix, re-run to Passed. E5 feed card and play screen preview for both kinds. E6 approve and send back. E7 correction of a published crossword | Pass |
| Publishing | P1 choose a Wordle for Sep 11 from the Approved pool. P2 schedule review with representative local times. P3 bulk schedule with per-date results incl. a skipped blocked date. P4 replace and unschedule with reasons; a day has exactly two slots and no ordering. P5 collection membership, metadata, visibility, shelf preview, save summary | Pass |
...
Cross-area checks: approve in Library then pick in the desk picker; grant tokens then read the ledger; import then read the batch in Operations; failed Daily game generation signal then land on Sep 11 in the desk. All pass. No user-visible "puzzle" wording remains; games are crosswords and Wordle, and the daily pair is the Daily game.
